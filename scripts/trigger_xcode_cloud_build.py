#!/usr/bin/env python3
"""Trigger and verify this app's App Store Xcode Cloud build."""
...
def select_workflow(product_id: str) -> dict:
    ...
    if len(enabled) != 1:
        ...
        raise RuntimeError(
            ...
            "Dispatch again with the exact workflow name after confirming it distributes to App Store Connect."
        )
    return enabled[0]
...
def wait_for_run(run_id: str, expected_sha: str) -> None:
    timeout_minutes = int(os.environ.get("XCODE_CLOUD_RUN_TIMEOUT_MINUTES", "90"))
    ...
    while time.monotonic() < deadline:
        ...
        if progress != last_progress:
            print(f"Xcode Cloud run {run_id}: {progress} ({completion or 'in progress'})")
        ...
        if progress == "COMPLETE":
            if completion != "SUCCEEDED":
                raise RuntimeError(f"Xcode Cloud run {run_id} finished with status {completion or 'unknown'}")
            return
        time.sleep(30)
    raise RuntimeError(f"Xcode Cloud run {run_id} did not finish within {timeout_minutes} minutes")


def find_distribution_build(run_id: str) -> dict:
    timeout_minutes = int(os.environ.get("APP_STORE_PROCESSING_TIMEOUT_MINUTES", "45"))
    deadline = time.monotonic() + timeout_minutes * 60
    while time.monotonic() < deadline:
        ...
        for build in response.get("data", []):
            ...
            marketing_version = included_versions.get(relation.get("id"))
            if marketing_version is None and relation.get("id"):
                pre_release = api_request(f"/preReleaseVersions/{relation['id']}").get("data", {})
                marketing_version = pre_release.get("attributes", {}).get("version")
            if build_number != BUILD_NUMBER or marketing_version != MARKETING_VERSION:
                continue
            state = attributes.get("processingState", "UNKNOWN")
            if state == "VALID":
                return build
            if state == "INVALID":
                raise RuntimeError(f"App Store Connect rejected build {MARKETING_VERSION} ({BUILD_NUMBER})")
            print(f"App Store Connect is processing build {MARKETING_VERSION} ({BUILD_NUMBER}): {state}")
        time.sleep(30)
    raise RuntimeError(
        f"Xcode Cloud completed, but App Store Connect did not report a VALID build "
        f"{MARKETING_VERSION} ({BUILD_NUMBER}) within {timeout_minutes} minutes. "
        "Confirm that the selected Xcode Cloud workflow distributes to App Store Connect."
    )
...
def main() -> None:
    ...
    wait_for_run(run_id, expected_sha)
    build = find_distribution_build(run_id)
    attributes = build.get("attributes", {})
    summary = (
        ...
        f"- Processing state: `{attributes.get('processingState', 'VALID')}`"
    )
